(** Count-5 iso-class pattern 7 for the n=5 dispatcher.  Auto-generated. *)
...
Section EdgeCount5_c7.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_5_c7 :
  ...
  Proof.
    ...
    assert (HR_only : forall x y, R2 x y -> x <> y -> (x = p /\ y = q) \/ (x = p /\ y = r) \/ (x = p /\ y = s) \/ (x = q /\ y = r) \/ (x = t /\ y = r)).
    ...
    set (rk1 := fun z : B =>
      ...
      else if excluded_middle_informative (z = s) then 1 else 3).
    set (rk2 := fun z : B =>

(** Throwaway extractor: print the canonical 7-edge iso-class shapes among the
    count-7 (3-incomparable-pair) posets, by lexmin edge-encoding over all 120
    relabelings.  Run via [Eval vm_compute]; the output seeds the count-7
...
Definition is_c7_poset (a : list (option bool)) : bool :=
  is_poset_b (mat_of a) && Nat.eqb (edge_count_b (mat_of a)) 7.

Definition count7_canons : list (list nat) :=
  nodup (list_eq_dec Nat.eq_dec)
        (map (fun a => canon (mat_of a))
             (filter is_c7_poset count7_assigns)).

(* Decode an edge code n = 5*i+j into the pair (i, j). *)
Definition decode (n : nat) : nat * nat := (Nat.div n 5, Nat.modulo n 5).
Definition count7_shapes : list (list (nat * nat)) :=
  map (map decode) count7_canons.

Eval vm_compute in (List.length count7_canons).
Eval vm_compute in count7_shapes.
...
Eval vm_compute in (map (fun codes => find_realizer (mat_of_codes codes)) count7_canons).

(** ∧ + chain case for [edge_count_5 = 3].

    Given [edge_count_5 = 3] and five distinct elements [p, q, r, u, v]
    with [R2 p r], [R2 q r], [R2 u v], apply
    [n5_inv_V_plus_chain_two_realizer]. *)
...
Section EdgeCount3_inv_V_chain.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma inv_V_chain_closure :
  ...
  Proof.
    intros a b c d e p q r u v
           ...
           Hpq Hpr Hpu Hpv Hqr Hqu Hqv Hru Hrv Huv HRpr HRqr HRuv
           x y HRxy Hxy_neq.
    assert (Hsi1 : strict_indicator R2 p r = 1)
      by (apply strict_indicator_eq_1; assumption).
    assert (Hsi2 : strict_indicator R2 q r = 1)
      by (apply strict_indicator_eq_1; assumption).
  ...
  Lemma n5_edge_count_3_inv_V_chain :
  ...
  Proof.
    ...
    right.
    destruct (inv_V_chain_closure a b c d e p q r u v
                Hab Hac Had Hae Hbc Hbd Hbe Hcd Hce Hde Hcov Hec
                Hpq Hpr Hpu Hpv Hqr Hqu Hqv Hru Hrv Huv HRpr HRqr HRuv
                x y HRxy Hxy_neq)

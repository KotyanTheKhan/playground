(** Count-5 iso-class pattern 1 for the n=5 dispatcher.  Auto-generated. *)
...
Section EdgeCount5_c1.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_5_c1 :
  ...
  Proof.
    ...
    assert (HR_only : forall x y, R2 x y -> x <> y -> (x = p /\ y = q) \/ (x = p /\ y = r) \/ (x = p /\ y = s) \/ (x = t /\ y = q) \/ (x = t /\ y = r)).
    ...
    assert (V2r : rk2 r = 2) by (unfold rk2; repeat (destruct (excluded_middle_informative _); [solve [reflexivity | exfalso; congruence] | ]); solve [reflexivity | exfalso; congruence]).

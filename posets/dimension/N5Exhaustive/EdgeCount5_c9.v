(** Count-5 iso-class pattern 9 for the n=5 dispatcher.  Auto-generated. *)
...
Section EdgeCount5_c9.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_5_c9 :
  ...
  Proof.
    ...
    assert (HR_only : forall x y, R2 x y -> x <> y -> (x = p /\ y = q) \/ (x = p /\ y = r) \/ (x = q /\ y = r) \/ (x = s /\ y = q) \/ (x = s /\ y = r)).
    ...
    set (rk1 := fun z : B =>
      ...
      else if excluded_middle_informative (z = s) then 1 else 4).
    set (rk2 := fun z : B =>

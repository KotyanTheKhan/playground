(** Count-5 iso-class pattern 2 for the n=5 dispatcher.  Auto-generated. *)
...
Section EdgeCount5_c2.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_5_c2 :
  ...
  Proof.
    ...
    assert (HR_only : forall x y, R2 x y -> x <> y -> (x = p /\ y = q) \/ (x = p /\ y = r) \/ (x = p /\ y = s) \/ (x = q /\ y = r) \/ (x = t /\ y = s)).

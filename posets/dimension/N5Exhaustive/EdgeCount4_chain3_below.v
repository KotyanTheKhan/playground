(** Chain3 + pendant-from-bottom case for [edge_count_5 = 4].

    Class 12: edges (a,b),(a,c),(a,d),(b,c).  Chain a<b<c with
    transitively-forced (a,c), plus a pendant a<d.  Vertex e is
    isolated.  Calls [n5_pendant_plus_isolated_two_realizer]. *)
...
Section EdgeCount4_chain3_below.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma carrier_5_pick_1_avoiding_4_cb :
  ...
  Proof.
    ...
    assert (Hsi_ag : strict_indicator R2 alpha gamma = 1)
      by (apply strict_indicator_eq_1; assumption).
    assert (Hsi_ad : strict_indicator R2 alpha delta = 1)
      by (apply strict_indicator_eq_1; assumption).
  ...
  Lemma n5_edge_count_4_chain3_below :
  ...
  Proof.
    intros Hcard a b c d e
           ...
           Hab_n Hag_n Had_n Hbg_n Hbd_n Hgd_n
           HRab HRbg HRag HRad.
    destruct (carrier_5_pick_1_avoiding_4_cb a b c d e alpha beta gamma delta
                Hab Hac Had Hae Hbc Hbd Hbe Hcd Hce Hde Hcov
                Hab_n Hag_n Had_n Hbg_n Hbd_n Hgd_n)
    ...
    - intros x y HRxy.
      ...
      right.
      destruct (chain3_below_closure a b c d e alpha beta gamma delta
                  Hab Hac Had Hae Hbc Hbd Hbe Hcd Hce Hde Hcov Hec
                  Hab_n Hag_n Had_n Hbg_n Hbd_n Hgd_n
                  HRab HRbg HRag HRad x y HRxy Hxy_neq)

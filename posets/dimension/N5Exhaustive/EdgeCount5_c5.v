(** Count-5 iso-class pattern 5 for the n=5 dispatcher.  Auto-generated. *)
...
Section EdgeCount5_c5.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_5_c5 :
  ...
  Proof.
    ...
    assert (HR_only : forall x y, R2 x y -> x <> y -> (x = p /\ y = q) \/ (x = p /\ y = r) \/ (x = s /\ y = q) \/ (x = s /\ y = r) \/ (x = t /\ y = q)).
    ...
    assert (V1q : rk1 q = 4) by (unfold rk1; repeat (destruct (excluded_middle_informative _); [solve [reflexivity | exfalso; congruence] | ]); solve [reflexivity | exfalso; congruence]).

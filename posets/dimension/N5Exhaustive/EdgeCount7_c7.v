(** Count-7 iso-class pattern 7 for the n=5 dispatcher.  Auto-generated
...
Section EdgeCount7_c7.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_7_c7 :
  ...
  Proof.
    intros Hcard a b c d e
           ...
           HRpq HRpr HRps HRqr HRsr HRtq HRtr.
    ...
    assert (HR_only : forall x y, R2 x y -> x <> y -> (x = p /\ y = q) \/ (x = p /\ y = r) \/ (x = p /\ y = s) \/ (x = q /\ y = r) \/ (x = s /\ y = r) \/ (x = t /\ y = q) \/ (x = t /\ y = r)).
    ...
    set (rk1 := fun z : B =>
      ...
      else if excluded_middle_informative (z = s) then 1 else 2).
    set (rk2 := fun z : B =>
    ...
    assert (V1q : rk1 q = 3) by (unfold rk1; repeat (destruct (excluded_middle_informative _); [solve [reflexivity | exfalso; congruence] | ]); solve [reflexivity | exfalso; congruence]).
    assert (V1r : rk1 r = 4) by (unfold rk1; repeat (destruct (excluded_middle_informative _); [solve [reflexivity | exfalso; congruence] | ]); solve [reflexivity | exfalso; congruence]).
    assert (V1s : rk1 s = 1) by (unfold rk1; repeat (destruct (excluded_middle_informative _); [solve [reflexivity | exfalso; congruence] | ]); solve [reflexivity | exfalso; congruence]).

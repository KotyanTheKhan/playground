(** Count-5 iso-class pattern 10 for the n=5 dispatcher.  Auto-generated. *)
...
Section EdgeCount5_c10.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_5_c10 :
  ...
  Proof.
    ...
    assert (HR_only : forall x y, R2 x y -> x <> y -> (x = p /\ y = q) \/ (x = p /\ y = r) \/ (x = q /\ y = r) \/ (x = s /\ y = r) \/ (x = t /\ y = r)).

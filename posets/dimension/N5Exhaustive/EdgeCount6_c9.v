(** Count-6 iso-class pattern 9 for the n=5 dispatcher.  Auto-generated
...
Section EdgeCount6_c9.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_6_c9 :
  ...
  Proof.
    ...
    assert (HR_only : forall x y, R2 x y -> x <> y -> (x = p /\ y = q) \/ (x = p /\ y = r) \/ (x = p /\ y = s) \/ (x = q /\ y = r) \/ (x = q /\ y = s) \/ (x = r /\ y = s)).

(** Count-8 iso-class pattern 3 for the n=5 dispatcher.

    Edges (p<q,p<r,p<s,q<r,s<r,t<q,t<r,t<s); incomparable pairs {p,t},{q,s}.
    2-realizer: L1 = p<t<q<s<r, L2 = t<p<s<q<r (reverses {p,t} and {q,s}). *)
...
Section EdgeCount8_c3.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_8_c3 :
    ...
    forall (a b c d e : B),
      ...
      forall (p q r s t : B),
        ...
        R2 q r -> R2 s r -> R2 t q -> R2 t r -> R2 t s ->
  ...
  Proof.
    ...
    set (rk2 := fun z : B =>
      ...
      else if excluded_middle_informative (z = s) then 2 else 0).
    ...
    assert (V2s : rk2 s = 2) by (unfold rk2; repeat (destruct (excluded_middle_informative _); [solve [reflexivity | exfalso; congruence] | ]); solve [reflexivity | exfalso; congruence]).

(** Count-7 iso-class pattern 9 for the n=5 dispatcher.  Auto-generated
...
Section EdgeCount7_c9.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_7_c9 :
  ...
  Proof.
    intros Hcard a b c d e
           ...
           HRpq HRpr HRqr HRsq HRsr HRtq HRtr.
    ...
    assert (HR_only : forall x y, R2 x y -> x <> y -> (x = p /\ y = q) \/ (x = p /\ y = r) \/ (x = q /\ y = r) \/ (x = s /\ y = q) \/ (x = s /\ y = r) \/ (x = t /\ y = q) \/ (x = t /\ y = r)).

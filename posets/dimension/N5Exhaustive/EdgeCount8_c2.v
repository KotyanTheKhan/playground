(** Count-8 iso-class pattern 2 for the n=5 dispatcher.

    Edges (p<q,p<r,p<s,q<r,q<s,t<q,t<r,t<s); incomparable pairs {p,t},{r,s}.
    p,t are the two minima; q the middle; r,s the two maxima.
    2-realizer: L1 = p<t<q<r<s, L2 = t<p<q<s<r (reverses {p,t} and {r,s}). *)
...
Section EdgeCount8_c2.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_8_c2 :
    ...
    forall (a b c d e : B),
      ...
      forall (p q r s t : B),
        ...
        R2 q r -> R2 q s -> R2 t q -> R2 t r -> R2 t s ->
  ...
  Proof.
    ...
    { intros x y HRxy Hxyne.
      ...
      assert (strict_indicator R2 t q = 1) by (apply strict_indicator_eq_1; (assumption || congruence)).
    ...
    set (rk2 := fun z : B =>
      ...
      else if excluded_middle_informative (z = s) then 3 else 0).
    ...
    assert (V1q : rk1 q = 2) by (unfold rk1; repeat (destruct (excluded_middle_informative _); [solve [reflexivity | exfalso; congruence] | ]); solve [reflexivity | exfalso; congruence]).
    ...
    assert (V1t : rk1 t = 1) by (unfold rk1; repeat (destruct (excluded_middle_informative _); [solve [reflexivity | exfalso; congruence] | ]); solve [reflexivity | exfalso; congruence]).
    ...
    assert (V2s : rk2 s = 3) by (unfold rk2; repeat (destruct (excluded_middle_informative _); [solve [reflexivity | exfalso; congruence] | ]); solve [reflexivity | exfalso; congruence]).

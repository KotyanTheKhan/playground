(** Count-7 iso-class pattern 6 for the n=5 dispatcher.  Auto-generated
...
Section EdgeCount7_c6.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_7_c6 :
  ...
  Proof.
    intros Hcard a b c d e
           ...
           HRpq HRpr HRps HRpt HRqr HRsr HRtr.
    ...
    assert (HR_only : forall x y, R2 x y -> x <> y -> (x = p /\ y = q) \/ (x = p /\ y = r) \/ (x = p /\ y = s) \/ (x = p /\ y = t) \/ (x = q /\ y = r) \/ (x = s /\ y = r) \/ (x = t /\ y = r)).
    ...
    set (rk2 := fun z : B =>
      ...
      else if excluded_middle_informative (z = s) then 2 else 1).
    ...
    - exists q, s. rewrite V1q, V1s, V2q, V2s. split; lia.
  Qed.

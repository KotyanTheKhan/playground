(** Count-8 iso-class pattern 1 for the n=5 dispatcher.

    Edges (p<q,p<r,p<s,p<t,q<r,q<s,t<r,t<s); incomparable pairs {q,t},{r,s}.
    p is the minimum; q,t are the two middle elements; r,s the two maxima.
    2-realizer: L1 = p<q<t<r<s, L2 = p<t<q<s<r (reverses {q,t} and {r,s}).

    The "closure" (R2's strict edges are exactly these 8) is a light 25-case
    enumeration: by [edge_count_5_cover_invariant] the count is 8 over p..t, so
    any 9th strict edge would make 9 distinct indicators equal 1 in a sum of 20
    that is 8 — contradiction (no 5^7 blow-up). *)
...
Section EdgeCount8_c1.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_8_c1 :
  ...
  Proof.
    ...
    set (rk2 := fun z : B =>
      ...
      else if excluded_middle_informative (z = s) then 3 else 1).

(** Count-7 iso-class pattern 1 for the n=5 dispatcher.  Auto-generated
...
Section EdgeCount7_c1.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_7_c1 :
  ...
  Proof.
    intros Hcard a b c d e
           ...
           HRpq HRpr HRps HRpt HRqr HRqs HRqt.
    ...
    assert (HR_only : forall x y, R2 x y -> x <> y -> (x = p /\ y = q) \/ (x = p /\ y = r) \/ (x = p /\ y = s) \/ (x = p /\ y = t) \/ (x = q /\ y = r) \/ (x = q /\ y = s) \/ (x = q /\ y = t)).
    ...
    set (rk2 := fun z : B =>
      ...
      else if excluded_middle_informative (z = s) then 3 else 2).
    ...
    - exists r, s. rewrite V1r, V1s, V2r, V2s. split; lia.
  Qed.

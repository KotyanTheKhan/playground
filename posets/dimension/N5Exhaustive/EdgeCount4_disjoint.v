(** Disjoint chain3 + chain2 case for [edge_count_5 = 4].

    Class 15: edges (a,b),(b,c),(a,c),(d,e).  Chain3 a<b<c (transitively
    closed) plus disjoint chain d<e.  Calls
    [n5_disjoint_chain3_chain2_two_realizer]. *)
...
Section EdgeCount4_disjoint.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma disjoint_closure :
  ...
  Proof.
    ...
    assert (Hsi_bc : strict_indicator R2 beta gamma = 1)
      by (apply strict_indicator_eq_1; assumption).
    assert (Hsi_ac : strict_indicator R2 alpha gamma = 1)
      by (apply strict_indicator_eq_1; assumption).
    assert (Hsi_de : strict_indicator R2 delta eps = 1)
      by (apply strict_indicator_eq_1; assumption).
  ...
  Lemma n5_edge_count_4_disjoint :
  ...
  Proof.
    ...
    right.
    destruct (disjoint_closure a b c d e alpha beta gamma delta eps
                Hab Hac Had Hae Hbc Hbd Hbe Hcd Hce Hde Hcov Hec
                H1 H2 H3 H4 H5 H6 H7 H8 H9 H10
                HRab HRbc HRac HRde x y HRxy Hxy_neq)

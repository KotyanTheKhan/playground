...
Section Iff.
  ...
  (** ---- count-8 iso-class patterns (8 edges each) ---- *)
  ...
  Ltac c8_finish Hex :=
    ...
    destruct Hpi_forall as [H1 [H2 [H3 [H4 [H5 [H6 [H7 [H8 _]]]]]]]];
    apply strict_b_R2_matrix_iff in H1, H2, H3, H4, H5, H6, H7, H8;
    only 1: (
      destruct H1 as [HR1 _]; destruct H2 as [HR2x _]; destruct H3 as [HR3 _];
      destruct H4 as [HR4 _]; destruct H5 as [HR5 _]; destruct H6 as [HR6 _];
      destruct H7 as [HR7 _]; destruct H8 as [HR8 _];
      pose proof (perm_in_all_perms5_nodup _ Hpi_in) as Hnd;
  ...
  Proof.
    unfold is_c8_1_b, has_edges_of_shape.
    intro Hex. apply existsb_exists in Hex.
    destruct Hex as [pi [Hpi_in Hpi_forall]].
    destruct pi as [|v1 [|v2 [|v3 [|v4 [|v5 [|? ?]]]]]]; try discriminate.
    simpl in Hpi_forall.
    ...
    destruct Hpi_forall as [H1 [H2 [H3 [H4 [H5 [H6 [H7 [H8 _]]]]]]]].
    apply strict_b_R2_matrix_iff in H1, H2, H3, H4, H5, H6, H7, H8.
    destruct H1 as [HR1 _]; destruct H2 as [HRm2 _]; destruct H3 as [HR3 _];
    destruct H4 as [HR4 _]; destruct H5 as [HR5 _]; destruct H6 as [HR6 _];
    destruct H7 as [HR7 _]; destruct H8 as [HR8 _].
    pose proof (perm_in_all_perms5_nodup _ Hpi_in) as Hnd.
    pose proof (NoDup_from_fin (B:=B) a b c d e Hab Hac Had Hae Hbc Hbd Hbe Hcd Hce Hde v1 v2 v3 v4 v5 Hnd) as HndB.
  ...
  Lemma is_c8_2_b_to_exists :
    is_c8_2_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 q r /\ R2 q s /\ R2 t q /\ R2 t r /\ R2 t s.
  Proof. unfold is_c8_2_b, has_edges_of_shape. intro Hex. c8_finish Hex. Qed.

  Lemma is_c8_3_b_to_exists :
    is_c8_3_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 q r /\ R2 s r /\ R2 t q /\ R2 t r /\ R2 t s.
  Proof. unfold is_c8_3_b, has_edges_of_shape. intro Hex. c8_finish Hex. Qed.

  Lemma is_c8_4_b_to_exists :
    is_c8_4_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 p t /\ R2 q r /\ R2 q s /\ R2 q t /\ R2 r s.
  Proof. unfold is_c8_4_b, has_edges_of_shape. intro Hex. c8_finish Hex. Qed.

  Lemma is_c8_5_b_to_exists :
    is_c8_5_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 p t /\ R2 q r /\ R2 q s /\ R2 r s /\ R2 t s.
  Proof. unfold is_c8_5_b, has_edges_of_shape. intro Hex. c8_finish Hex. Qed.

  Lemma is_c8_6_b_to_exists :
    is_c8_6_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 q r /\ R2 q s /\ R2 r s /\ R2 t r /\ R2 t s.
  Proof. unfold is_c8_6_b, has_edges_of_shape. intro Hex. c8_finish Hex. Qed.


  (** ---- count-7 iso-class patterns (7 edges each) ---- *)
  Ltac c7_finish Hex :=
    apply existsb_exists in Hex;
    destruct Hex as [pi [Hpi_in Hpi_forall]];
    destruct pi as [|v1 [|v2 [|v3 [|v4 [|v5 [|? ?]]]]]]; try discriminate;
    simpl in Hpi_forall; rewrite !andb_true_iff in Hpi_forall;
    destruct Hpi_forall as [H1 [H2 [H3 [H4 [H5 [H6 [H7 _]]]]]]];
    apply strict_b_R2_matrix_iff in H1, H2, H3, H4, H5, H6, H7;
    only 1: (
      destruct H1 as [HR1 _]; destruct H2 as [HR2x _]; destruct H3 as [HR3 _];
      destruct H4 as [HR4 _]; destruct H5 as [HR5 _]; destruct H6 as [HR6 _];
      destruct H7 as [HR7 _];
      pose proof (perm_in_all_perms5_nodup _ Hpi_in) as Hnd;
      pose proof (NoDup_from_fin (B:=B) a b c d e Hab Hac Had Hae Hbc Hbd Hbe Hcd Hce Hde
                    v1 v2 v3 v4 v5 Hnd) as HndB;
      pose proof (NoDup5_pairwise _ _ _ _ _ HndB) as Hpairwise;
  ...
  Lemma is_c7_1_b_to_exists :
    is_c7_1_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 p t /\ R2 q r /\ R2 q s /\ R2 q t.
  Proof. unfold is_c7_1_b, has_edges_of_shape. intro Hex. c7_finish Hex. Qed.

  Lemma is_c7_2_b_to_exists :
    is_c7_2_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 q r /\ R2 q s /\ R2 t r /\ R2 t s.
  Proof. unfold is_c7_2_b, has_edges_of_shape. intro Hex. c7_finish Hex. Qed.

  Lemma is_c7_3_b_to_exists :
    is_c7_3_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 p t /\ R2 q r /\ R2 q s /\ R2 t r.
  Proof. unfold is_c7_3_b, has_edges_of_shape. intro Hex. c7_finish Hex. Qed.

  Lemma is_c7_4_b_to_exists :
    is_c7_4_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 q r /\ R2 t q /\ R2 t r /\ R2 t s.
  Proof. unfold is_c7_4_b, has_edges_of_shape. intro Hex. c7_finish Hex. Qed.

  Lemma is_c7_5_b_to_exists :
    is_c7_5_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 p t /\ R2 q r /\ R2 q s /\ R2 r s.
  Proof. unfold is_c7_5_b, has_edges_of_shape. intro Hex. c7_finish Hex. Qed.

  Lemma is_c7_6_b_to_exists :
    is_c7_6_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 p t /\ R2 q r /\ R2 s r /\ R2 t r.
  Proof. unfold is_c7_6_b, has_edges_of_shape. intro Hex. c7_finish Hex. Qed.

  Lemma is_c7_7_b_to_exists :
    is_c7_7_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 q r /\ R2 s r /\ R2 t q /\ R2 t r.
  Proof. unfold is_c7_7_b, has_edges_of_shape. intro Hex. c7_finish Hex. Qed.

  Lemma is_c7_8_b_to_exists :
    is_c7_8_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 q r /\ R2 q s /\ R2 r s /\ R2 t s.
  Proof. unfold is_c7_8_b, has_edges_of_shape. intro Hex. c7_finish Hex. Qed.

  Lemma is_c7_9_b_to_exists :
    is_c7_9_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 q r /\ R2 s q /\ R2 s r /\ R2 t q /\ R2 t r.
  Proof. unfold is_c7_9_b, has_edges_of_shape. intro Hex. c7_finish Hex. Qed.


  (** ---- count-6 iso-class patterns (6 edges each) ---- *)
  Ltac c6_finish Hex :=
    apply existsb_exists in Hex;
    destruct Hex as [pi [Hpi_in Hpi_forall]];
    destruct pi as [|v1 [|v2 [|v3 [|v4 [|v5 [|? ?]]]]]]; try discriminate;
    simpl in Hpi_forall; rewrite !andb_true_iff in Hpi_forall;
    destruct Hpi_forall as [H1 [H2 [H3 [H4 [H5 [H6 _]]]]]];
    apply strict_b_R2_matrix_iff in H1, H2, H3, H4, H5, H6;
    only 1: (
      destruct H1 as [HR1 _]; destruct H2 as [HR2x _]; destruct H3 as [HR3 _];
      destruct H4 as [HR4 _]; destruct H5 as [HR5 _]; destruct H6 as [HR6 _];
      pose proof (perm_in_all_perms5_nodup _ Hpi_in) as Hnd;
  ...
  Lemma is_c6_1_b_to_exists :
    is_c6_1_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 t q /\ R2 t r /\ R2 t s.
  Proof. unfold is_c6_1_b, has_edges_of_shape. intro Hex. c6_finish Hex. Qed.

  Lemma is_c6_2_b_to_exists :
    is_c6_2_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 p t /\ R2 q r /\ R2 s t.
  Proof. unfold is_c6_2_b, has_edges_of_shape. intro Hex. c6_finish Hex. Qed.

  Lemma is_c6_3_b_to_exists :
    is_c6_3_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 p t /\ R2 q r /\ R2 q s.
  Proof. unfold is_c6_3_b, has_edges_of_shape. intro Hex. c6_finish Hex. Qed.

  Lemma is_c6_4_b_to_exists :
    is_c6_4_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 s q /\ R2 s r /\ R2 t q /\ R2 t r.
  Proof. unfold is_c6_4_b, has_edges_of_shape. intro Hex. c6_finish Hex. Qed.

  Lemma is_c6_5_b_to_exists :
    is_c6_5_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 q r /\ R2 t r /\ R2 t s.
  Proof. unfold is_c6_5_b, has_edges_of_shape. intro Hex. c6_finish Hex. Qed.

  Lemma is_c6_6_b_to_exists :
    is_c6_6_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 p t /\ R2 q r /\ R2 s r.
  Proof. unfold is_c6_6_b, has_edges_of_shape. intro Hex. c6_finish Hex. Qed.

  Lemma is_c6_7_b_to_exists :
    is_c6_7_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 q r /\ R2 q s /\ R2 t r.
  Proof. unfold is_c6_7_b, has_edges_of_shape. intro Hex. c6_finish Hex. Qed.

  Lemma is_c6_8_b_to_exists :
    is_c6_8_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 q r /\ R2 t q /\ R2 t r.
  Proof. unfold is_c6_8_b, has_edges_of_shape. intro Hex. c6_finish Hex. Qed.

  Lemma is_c6_9_b_to_exists :
    is_c6_9_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 q r /\ R2 q s /\ R2 r s.
  Proof. unfold is_c6_9_b, has_edges_of_shape. intro Hex. c6_finish Hex. Qed.

  Lemma is_c6_10_b_to_exists :
    is_c6_10_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 q r /\ R2 s r /\ R2 s t /\ R2 t r.
  Proof. unfold is_c6_10_b, has_edges_of_shape. intro Hex. c6_finish Hex. Qed.

  Lemma is_c6_11_b_to_exists :
    is_c6_11_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\
      R2 p q /\ R2 p r /\ R2 p s /\ R2 q r /\ R2 s r /\ R2 t r.
  Proof. unfold is_c6_11_b, has_edges_of_shape. intro Hex. c6_finish Hex. Qed.

  Lemma is_c6_12_b_to_exists :
    is_c6_12_b M = true ->
    exists p q r s t : B,
      p <> q /\ p <> r /\ p <> s /\ p <> t /\ q <> r /\ q <> s /\ q <> t /\
      r <> s /\ r <> t /\ s <> t /\

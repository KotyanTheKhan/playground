(** Count-6 iso-class pattern 4 for the n=5 dispatcher.  Auto-generated
...
Section EdgeCount6_c4.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_6_c4 :
  ...
  Proof.
    ...
    assert (HR_only : forall x y, R2 x y -> x <> y -> (x = p /\ y = q) \/ (x = p /\ y = r) \/ (x = s /\ y = q) \/ (x = s /\ y = r) \/ (x = t /\ y = q) \/ (x = t /\ y = r)).
    { intros x y HRxy Hxyne.
      ...
      assert (strict_indicator R2 s q = 1) by (apply strict_indicator_eq_1; (assumption || congruence)).
    ...
    set (rk2 := fun z : B =>
      ...
      else if excluded_middle_informative (z = s) then 1 else 0).
    ...
    assert (V2s : rk2 s = 1) by (unfold rk2; repeat (destruct (excluded_middle_informative _); [solve [reflexivity | exfalso; congruence] | ]); solve [reflexivity | exfalso; congruence]).

(** Count-8 iso-class pattern 4 for the n=5 dispatcher.

    Edges (p<q,p<r,p<s,p<t,q<r,q<s,q<t,r<s); incomparable pairs {r,t},{s,t}.
    Chain p<q<r<s with t between q and the top (p<t,q<t), t incomparable to r,s.
    2-realizer: L1 = p<q<r<s<t, L2 = p<q<t<r<s (reverses {r,t} and {s,t}). *)
...
Section EdgeCount8_c4.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_8_c4 :
  ...
  Proof.
    ...
    { intros x y HRxy Hxyne.
      ...
      assert (strict_indicator R2 q t = 1) by (apply strict_indicator_eq_1; (assumption || congruence)).
    ...
    set (rk1 := fun z : B =>
      ...
      else if excluded_middle_informative (z = s) then 3 else 4).
    set (rk2 := fun z : B =>
      if excluded_middle_informative (z = p) then 0
      else if excluded_middle_informative (z = q) then 1
    ...
    assert (V1s : rk1 s = 3) by (unfold rk1; repeat (destruct (excluded_middle_informative _); [solve [reflexivity | exfalso; congruence] | ]); solve [reflexivity | exfalso; congruence]).
    ...
    assert (V2q : rk2 q = 1) by (unfold rk2; repeat (destruct (excluded_middle_informative _); [solve [reflexivity | exfalso; congruence] | ]); solve [reflexivity | exfalso; congruence]).
    ...
    assert (V2t : rk2 t = 2) by (unfold rk2; repeat (destruct (excluded_middle_informative _); [solve [reflexivity | exfalso; congruence] | ]); solve [reflexivity | exfalso; congruence]).
    ...
    - exists r, t. rewrite V1r, V1t, V2r, V2t. split; lia.
  Qed.

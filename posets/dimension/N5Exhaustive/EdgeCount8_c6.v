(** Count-8 iso-class pattern 6 for the n=5 dispatcher.

    Edges (p<q,p<r,p<s,q<r,q<s,r<s,t<r,t<s); incomparable pairs {p,t},{q,t}.
    Chain p<q<r<s with t below r,s (t<r,t<s), t incomparable to p,q.
    2-realizer: L1 = p<q<t<r<s, L2 = t<p<q<r<s (reverses {p,t} and {q,t}). *)
...
Section EdgeCount8_c6.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_8_c6 :
    ...
    forall (a b c d e : B),
      ...
      forall (p q r s t : B),
        ...
        R2 p q -> R2 p r -> R2 p s ->
        R2 q r -> R2 q s -> R2 r s -> R2 t r -> R2 t s ->
  ...
  Proof.
    ...
    { intros x y HRxy Hxyne.
      ...
      assert (strict_indicator R2 t r = 1) by (apply strict_indicator_eq_1; (assumption || congruence)).
    ...
    set (rk1 := fun z : B =>
      ...
      else if excluded_middle_informative (z = s) then 4 else 2).
    set (rk2 := fun z : B =>
    ...
    assert (V1t : rk1 t = 2) by (unfold rk1; repeat (destruct (excluded_middle_informative _); [solve [reflexivity | exfalso; congruence] | ]); solve [reflexivity | exfalso; congruence]).

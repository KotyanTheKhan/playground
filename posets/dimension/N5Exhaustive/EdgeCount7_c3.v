(** Count-7 iso-class pattern 3 for the n=5 dispatcher.  Auto-generated
...
Section EdgeCount7_c3.
  Context {B : Type}.
  Context (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}.

  Lemma n5_edge_count_7_c3 :
  ...
  Proof.
    intros Hcard a b c d e
           ...
           HRpq HRpr HRps HRpt HRqr HRqs HRtr.
    ...
    assert (HR_only : forall x y, R2 x y -> x <> y -> (x = p /\ y = q) \/ (x = p /\ y = r) \/ (x = p /\ y = s) \/ (x = p /\ y = t) \/ (x = q /\ y = r) \/ (x = q /\ y = s) \/ (x = t /\ y = r)).
    ...
    set (rk1 := fun z : B =>
      ...
      else if excluded_middle_informative (z = s) then 2 else 3).
    set (rk2 := fun z : B =>
      if excluded_middle_informative (z = p) then 0

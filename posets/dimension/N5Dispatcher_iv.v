...
From Dimension Require Import N5Dispatcher_iv_a.
...
(** Micro-case (iv) of the second-edge cascade inside the residual handler:
    ...
    [s] — contradicting [HnDisj].  The 18-case third-edge cascade is split
    across N5Dispatcher_iv_a.v (cases a-f), _iv_b.v (g-l) and _iv_c.v
...
Proof.
  ...
  - (* A third strict edge exists - delegate to subcase_a. *)
    apply (@n5_dispatcher_microcase_iv_subcase_a B R2 HR2 Hcard

...
From Dimension Require Import N5Dispatcher_ii_a.
...
(** Micro-case (ii) of the second-edge cascade inside the residual handler:
    ...
    isolated [t] — contradicting [HnDisj].  The 18-case third-edge cascade
    is split across N5Dispatcher_ii_a.v (cases a-f), _ii_b.v (g-l) and
    _ii_c.v (o, p, m, n + antisymmetry + final exfalso); this file is the
    thin wrapper. *)
...
Proof.
  ...
  destruct (classic (exists a b : B,
            ...
            ~ (a = p /\ b = q) /\ ~ (a = r /\ b = s)))
    as [Hthird | Hno_third].
  - (* A third strict edge exists - delegate to subcase_a. *)
    apply (@n5_dispatcher_microcase_ii_subcase_a B R2 HR2 Hcard

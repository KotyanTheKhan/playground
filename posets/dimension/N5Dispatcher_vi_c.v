...
(** Third sub-block of micro-case (vi): 3rd-edge cases (m) [= (q, t) in
    src order labeled (o)], (n) [= (t, q)], (o) [= (p, t)], (p) [= (s, q)],
    plus the antisymmetry cases (q), (r) and the final exfalso that uses
    the full bag of 18 negation hypotheses to contradict [Hthird] via the
    [Hcov5] case split. *)
Lemma n5_dispatcher_microcase_vi_subcase_c :
  forall {B : Type} (R2 : B -> B -> Prop) `{HR2 : IsPoset B R2}
    ...
    (Hthird :
       exists a b : B,
         a <> b /\ R2 a b /\
         ~ (a = p /\ b = q) /\ ~ (a = s /\ b = t))
    (HnRqr : ~ R2 q r) (HnRrp : ~ R2 r p) (HnRtr : ~ R2 t r)
    (HnRrs : ~ R2 r s) (HnRpr : ~ R2 p r) (HnRrq : ~ R2 r q)
    (HnRsr : ~ R2 s r) (HnRrt_third : ~ R2 r t)
    (HnRqs : ~ R2 q s) (HnRtp : ~ R2 t p)
    (HnRps : ~ R2 p s) (HnRsp : ~ R2 s p),
...
Proof.
  intros B R2 HR2 Hcard Hnonantichain Hinc_ex p q r s t
         ...
         HnRsr HnRrt_third HnRqs HnRtp HnRps HnRsp.

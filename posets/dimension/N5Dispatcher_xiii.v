...
From Dimension Require Import N5Dispatcher_xiii_a.
...
(** Micro-case (xiii) of the second-edge cascade inside the residual handler:
    ...
    isolated [s], [r].  If no third strict edge exists, this contradicts
    ...
    matching upstream per-class shape lemma.  The 18-case cascade is
    split across N5Dispatcher_xiii_a.v (cases a-f), _xiii_b.v (g-l) and
    _xiii_c.v (m-r + final exfalso); this file is the thin wrapper. *)
...
Proof.
  ...
  destruct (classic (exists a b : B,
            ...
            ~ (a = p /\ b = q) /\ ~ (a = t /\ b = q)))
    as [Hthird | Hno_third].
  - apply (@n5_dispatcher_microcase_xiii_subcase_a B R2 HR2 Hcard

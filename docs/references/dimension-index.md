...
_71 entries (16 DEF, 13 FACT, 36 THM, 5 ALG, 1 CONJ). Auto-generated — do not edit between the markers._
...
1. [Foundations](#1-foundations)
   ...
   - `[FACT] dim-chain` — Chains have dimension ≤ 1
...
5. [Computational complexity](#5-computational-complexity)
   ...
   - `[THM] gallai-comparability` — Forbidden-subgraph characterization of comparability graphs
   - `[ALG] dim-approx-hard` — Dimension is hard to approximate
...
8. [Dimension and chromatic number](#8-dimension-and-chromatic-number)
   ...
   - `[THM] boxicity-dim` — Dimension is bounded by boxicity
9. [Application: logical clocks (project goal)](#9-application-logical-clocks-project-goal)
   ...
   - `[FACT] fully-sync-dim2` — Fully-synchronized executions are dimension ≤ 2
   - `[FACT] sync-connectivity` — Connectivity conditions force synchronization
...
**Depends on.** [realizer]
**Sources.** [DM41].
**In this repo.** `PosetDimension` — `posets/dimension/DimDefs.v`; well-definedness `dimension_unique` (the dimension is unique) — `posets/dimension/DimBasics.v`.
...
#### [FACT] dim-chain — Chains have dimension ≤ 1
**Statement.** A finite poset has dimension ≤ 1 iff it is a chain (total order). A total order is its own one-element realizer, so dim ≤ 1; conversely a one-element realizer is a single linear extension equal to the order, which is therefore total.
**Status.** Proven. Sources: [DM41]; [T92].
**Depends on.** [dim-def], [realizer]
**In this repo.** `total_order_is_realizer` + `total_order_dim_le_1` (chain ⇒ dim ≤ 1), admit-free — `posets/dimension/DimBasics.v`.
**Notes.** The base case below [dim2-comparability]; the cheapest possible clock (one coordinate) is exactly a totally-ordered execution.
...
**In this repo.** *Fully proven, admit-free* — `posets/dilworth/Mirsky.v` (rank layer: `posets/FinPosetRank.v`, `posets/FinPosetWF.v`, `posets/FiniteMax.v`). Headline `mirsky` (finite poset, height ≥ 1): the maximum chain size and the minimum antichain-cover size both equal `height` (:= max rank). Built from: lower bound `chain_le_antichain_cover`; a longest-chain rank function via well-founded recursion (`rank`, `rank_strict_mono`, `rank_level_antichain`, contiguity `rank_achieves`); the rank-level cover `mirsky_cover` (`mirsky_cover_is_cover` + `mirsky_cover_cardinal` = height); and a height-realizing chain `exists_chain_height` (`chain_to_x`).
...
**Status.** Definition.
**Sources.** [DM41]; [T92].
**In this repo.** `StandardExampleRel n k` (the S(n,k) family) — `standard_example/Structure.v` (`StandardExample_Not_Lattice` etc.).
...
#### [THM] gallai-comparability — Forbidden-subgraph characterization of comparability graphs
**Statement.** Gallai (1967) characterized comparability graphs (transitively orientable graphs) by a finite list of minimal forbidden induced subgraphs (19 families), together with their modular decomposition. Equivalently: a graph is a comparability graph iff every odd closed walk has a triangular chord.
**Status.** Proven (Gallai 1967). Sources: [Gallai67]; [T92].
**Depends on.** [dim2-comparability], [transitive-orientation]
**Notes.** The structural backbone behind comparability-graph recognition (hence dim ≤ 2 testing).

#### [ALG] dim-approx-hard — Dimension is hard to approximate
**Statement.** Unless P = NP (in fact unless NP ⊆ ZPP), there is no polynomial-time algorithm approximating the dimension of an n-element poset within a factor of n^{1−ε} for any ε > 0 (Chalermsook–Laekhanukit–Nanongkai 2013, via graph products).
**Status.** Proven (Chalermsook–Laekhanukit–Nanongkai 2013). Sources: [CLN13].
**Depends on.** [dim3-npc]
**Notes.** Strengthens [dim3-npc]: not only exact computation but even coarse approximation of dimension is intractable in the worst case — so cheap clocks must come from structure, not generic approximation.
...
#### [THM] boxicity-dim — Dimension is bounded by boxicity
**Statement.** dim(P) ≤ 2·box(G_P), where box(G_P) is the boxicity of the comparability graph of P (the least k for which G_P is an intersection graph of axis-parallel boxes in ℝ^k). Combined with box(G) ≤ tw(G) + 2 this gives dim(P) ≤ 2·tw(G_P) + 4 (Adiga–Bhowmick–Chandran 2011).
**Status.** Proven (Adiga–Bhowmick–Chandran 2011). Sources: [ABC11].
**Depends on.** [dim-def], [dim-geometric]
**Notes.** A geometric companion to [dim-geometric]: dimension (chain-product embedding) and boxicity (box intersection) are within a factor 2 on comparability graphs.
...
#### [FACT] fully-sync-dim2 — Fully-synchronized executions are dimension ≤ 2
**Statement.** A fully-synchronized execution whose blocks each contain no alternating cycle has dimension ≤ 2; more generally, the dimension of a block-decomposed (ordinal-partitioned) execution equals the maximum of its block dimensions. So a synchronizing execution with reversible blocks admits a 2-coordinate clock.
**Status.** Proven (this repository; admit-free). Sources: [T92].
**Depends on.** [exec-poset-low-dim], [dim-via-critical-pairs]
**In this repo.** `fully_sync_dim2` — `execution/sync/FullySync.v`; `fully_sync_dimension` (dimension = max of block dimensions) — `execution/sync/FullySyncDimExact.v`.

#### [FACT] sync-connectivity — Connectivity conditions force synchronization
**Statement.** Structural connectivity of a schedule discharges the synchronization hypotheses behind the dimension-≤2 results: windowed barrier connectivity yields cross-window causal ordering for an arbitrary number of processes, and a step-connected schedule is fully synchronizing.
**Status.** Proven (this repository; admit-free). Sources: [T92].
**Depends on.** [fully-sync-dim2], [exec-poset-low-dim]
**In this repo.** `window_connected_barrier` — `execution/barriers/WindowSync.v`; `step_connected_fully_synchronizing` — `execution/barriers/ConnSync.v` (memory notes `project_window_barrier`, `project_connectivity_fullsync`).

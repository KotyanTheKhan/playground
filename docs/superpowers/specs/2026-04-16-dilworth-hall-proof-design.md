...
Standalone Hall's marriage theorem. No dependency on Dilworth-specific definitions.
...
4. **Maximum matching = `|sub| - w`:**
   - `≥`: augmented-graph Hall's (add `w` dummies connected to all of `X`; Hall's condition holds by fact 2).
...
Lemma matching_to_assignment : forall (sub la : Ensemble A) w (m : A -> A),
  IsLargestAntichain R sub la w ->
  Included A sub (Above R la) ->
  (* m is a perfect matching on the augmented Dilworth graph *)
  (* ... matching hypotheses ... *)
  exists f : A -> A,
...
- `Hall.v` compiles standalone (no Dilworth-specific imports)
